...
// Mutex simple para sincronizar peticiones concurrentes
...
const fetchAsyncData = (data, ms = 10) => new Promise(resolve => setTimeout(() => resolve(data), ms));
...
export function createApp() {
  ...
  const items = [
    { id: 1, name: 'alpha', score: 10, capacity: 5 },
    { id: 2, name: 'beta', score: 20, capacity: 5 }
  ];
  ...
  app.get('/items', async (req, res, next) => {
    try {
      let result = await fetchAsyncData([...items]);
  ...
  app.get('/items/:id', async (req, res, next) => {
    try {
      ...
      const itemList = await fetchAsyncData(items);
      const item = itemList.find(current => current.id === idParam);
  ...
  // Endpoint de reserva con protección contra condición de carrera (Race Condition)
  app.post('/items/:id/reserve', async (req, res, next) => {
    const nextTask = lock.then(async () => {
      const idParam = Number(req.params.id);
      if (isNaN(idParam)) {
        return res.status(422).json({ error: 'unprocessable entity', requestId: req.requestId });
      }
      ...
      await fetchAsyncData(null, 10);

      if (item.capacity <= 0) {
        return res.status(409).json({ error: 'no capacity available', requestId: req.requestId });
      }

      item.capacity -= 1;
      return res.status(200).json({ success: true, remainingCapacity: item.capacity });
    });
  ...
  app.post('/items', async (req, res, next) => {
    try {
      ...
      const item = { id: newItemId, name, score, capacity: 5 };
      
      await fetchAsyncData(item);
      items.push(item);
